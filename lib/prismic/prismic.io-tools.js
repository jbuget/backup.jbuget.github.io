...
prismic.utils = {
    ...
    docCookies: {
        ...
        removeItem: function (sKey, sPath, sDomain) {
            if (!prismic.utils.docCookies.hasItem(sKey)) { return false; }
...
prismic.startExperiment = function(expId, cxApi) {
    try {
        if(!prismic.utils.docCookies.getItem(prismic.previewCookieKey)) {
            if(!prismic.utils.docCookies.supportCookies()) return;
            var storeVariationAndReload = function(variation) {
                prismic.utils.docCookies.setItem(prismic.experimentCookieKey, [expId, variation].join(' '), 60 * 60 * 24 * 30, "/");
...
prismic.toolbar = function() {
    ...
    try {
        ...
        var previewToken = prismic.utils.docCookies.getItem(prismic.previewCookieKey);

        // Fail fast, there is no preview available
        if(previewToken) {

            prismic.utils.loadJQuery(function($) {
                ...
                var closeSession = function() {
                    ...
                    for(var d=0; d<domainParts.length; d++) {
                        ...
                        for(var p=0; p<pathParts.length; p++) {
                            var path = pathParts.slice(0, pathParts.length - p).join('/');
                            prismic.utils.docCookies.removeItem(prismic.previewCookieKey, path + '/', domain.indexOf('.') > -1 ? domain : '');
                            prismic.utils.docCookies.removeItem(prismic.previewCookieKey, path, domain.indexOf('.') > -1 ? domain : '');
                        }
                    }
                };
        ...
        var experimentToken = prismic.utils.docCookies.getItem(prismic.experimentCookieKey);
